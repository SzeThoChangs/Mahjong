...
const CARDS: Tip[] = [
  ...
  },
  {
    ...
    why: [
      ...
      'Holding both costs nothing in distance here. The sixth *Block* is made of tiles that would otherwise be *Floaters*, so the hand is the same distance from *Ting Pai* either way.',
  ...
  },
  {
    ...
    verdict: 'measured',
    verdictNote: 'Counting agrees with the book \u2014 both hands are one away, and the third pair costs about a third of the accepting tiles. Playing it out does not. On 87 graded positions holding three pairs, the *Measured Best* throw was the *Floater*, keeping all three, 69% of the time against 34% expected from the number of tiles each side offers. That held whether the alternative was a *Floater* number tile or an *Honour*, and whether the hand was near *Ting Pai* or far from it. One reason may be the table: *Pong Pong* is 2 *Tai* here and the minimum is 2, so a third pair is a route to a hand you are allowed to declare, which no count of accepting tiles can see. Re-checked on 2026-09-05, and the failure was ours rather than the book’s. The 69% above is measured against a coin that equalises how many tiles sit on each side of the split, not what those tiles are, and the tile this card warns against throwing is the loose one — which is the *Measured Best* 69% of the time across every graded position in the packs, whatever any card says. Scored against a baseline that knows that, the tip comes out 0.8 standard errors HIGH on 592 resolved positions. So keeping a third pair is neither better nor worse than the throw a hand makes anyway, which is a different verdict from the one written above it. `audit.ts` is the tool.',
  },
  {
    id: 'sandwich',
    phase: 'build',
  ...
  },
  {
    ...
    rule: 'Five tiles in a row are not a set plus *Floaters*. They are a *Block* that can be taken apart and put back together on a better *Wait*, cheaply.',
    why: [
  ...
  },
  {
    ...
    why: [
      ...
      'It looks untidy because one tile is left over with nothing to do. That *Floater* is what the shape costs, not a fault in it.',
    ],
  ...
  },
  {
    ...
    phase: 'build',
    title: 'A 3 or a 7 is the best *Floater* to keep',
  ...
  },
  {
    ...
    rule: 'Do not lock yourself into the *Wait* that accepts the most tiles if the tiles it needs are already gone. A *Floater* tile that keeps the hand able to improve is often worth more.',
    why: [
      ...
      'A hand made entirely of *Blocks* has nothing left to improve with. The six-*Blocks* card above shows the size of that: 2 tiles widen the all-*Blocks* hand and 26 widen the one that kept two *Floaters*.',
    ],
    variants: [],
    claim: { kind: 'not-countable' },
    verdict: 'measured',
  ...
  },
  {
    ...
    why: [
      'A triplet can be a finished set or it can be a pair with a *Floater* copy attached, and those two readings give completely different *Waits*.',
  ...
  },
  {
    ...
    rule: '*Half Colour* is 2 *Tai*, *Full Colour* 4, *Pong Pong* 2, *Ping Wu* 4. Those come from the shape of the whole hand. A *Dragon* *Pong* or your *Seat Wind* is 1 *Tai* and depends on one *Block* surviving.',
    why: [
      'A whole-hand pattern cannot be taken away by one bad draw. A part-hand pattern is one broken *Block* away from nothing.',
      'It also stacks: a *Half Colour* hand can still hold a *Dragon* *Pong*, but the reverse plan rarely turns into a colour hand.',
    ],
  ...
  },
  {
    ...
    why: [
      ...
      'It is the same idea as keeping a *Floater* rather than a sixth *Block*, one *Turn* further ahead.',
    ],
    variants: [],
    claim: { kind: 'not-countable' },
    verdict: 'measured',
  ...
  },
  {
    ...
    rule: 'Do not open with a favourite. Score the hand for each of the plans available — the *Pi Wu*, the colour hand, the all-runs hand, the *Pong Pong* hand — and take whichever is worth the most right now.',
    why: [
  ...
  },
  {
    ...
    rule: 'Playing the same combination every hand loses money whichever one you pick: the *Pi Wu* -0.9 chips a game, all-runs -2.9, *Pong Pong* -4.4, *Half Colour* -4.8, *Full Colour* -8.8, 13 Wonders -9.1. Choosing between them is the skill.',
    why: [
  ...
  },
  {
    ...
    rule: 'At this table you may not declare a win under 2 *Tai* on a discard. That pushes the opening book toward the bigger hands: *Half Colour* 32% of hands, all-runs 26%, *Ping Wu* 24%, *Pong Pong* 14%, and the *Pi Wu* only 4%.',
    why: [
      'The *Pi Wu* is no longer a plan, because it is not legal to declare unless something else arms it. It survives as a fallback you keep alive, not as a target.',
  ...
  },
  {
    ...
    rule: 'Before throwing out of habit, compare your *Floaters*. Whether a copy has already been thrown matters more than what kind of tile it is.',
    why: [
  ...
  },
  {
    ...
    rule: 'A *Pong* can come from any of the three, not just the player after you. An *Honour* thrown at *Turn* 40 is claimed about 16% of the time against a *Pi Wu* and 29% against a colour hand.',
    why: [
  ...
  },
  {
    ...
    phase: 'discard',
    title: 'A *Pong Pong* hand runs out of safe tiles',
  ...
  },
  {
    ...
    why: [
      'What you keep in hand after a call is your defence, and a run in hand is more useful to defend with than a *Floater* pair.',
  ...
  },
  {
    ...
    rule: '*Pong Pong* is 2 *Tai*, and as a plan it measured -4.4 chips a game — the worst of the ordinary targets. Only steer into it when it arrives with something else: a *Dragon*, your *Seat Wind*, a colour, *Animals*.',
    why: [
      'It shortens your hand every time you call, so you end up with no defence, and it always finishes on a narrow *Wait*.',
      'The fix that measured well is to keep the *Pi Wu* alive underneath it as a fallback, which is worth +3.4 chips a game.',
    ],
    variants: [],
    claim: { kind: 'not-countable' },
    verdict: 'measured',
  ...
  },
  {
    id: 'hybrid_fallback',
    phase: 'call',
    title: 'Keep the *Pi Wu* alive underneath the plan',
    rule: 'Playing for triplets or for runs, keep the *Pi Wu* available and take it the moment it is legal. That fallback measured +3.4 chips a game.',
    why: [
  ...
  },
  {
    ...
    why: [
      'You cannot win a race you have not entered, and a *Pi Wu* that ends the round stops theirs.',
  ...
  },
  {
    ...
    phase: 'call',
    title: 'When everybody folds, widen for the *Zi Mo*',
    ...
    why: [
      'A *Zi Mo* needs only one *Tai* here, so a wide cheap *Wait* is a real hand once nobody is feeding you.',
  ...
  },
  {
    ...
    rule: 'Look at what they have exposed and add it up. A *Dragon* *Pong* or their *Seat Wind* is 1 *Tai* each, a colour is 2 or 4, visible *Animals* and *Flowers* 1 each. A *Pi Wu* you can afford to feed; an expensive one you cannot.',
    why: [
  ...
  },
  {
    ...
    rule: 'Somebody declaring a concealed *Kong* has a hand far enough along to *Floater* four tiles for one set, and they get paid for it immediately. Treat them as a live threat and start banking safe tiles.',
    why: [
  ...
  },
  {
    ...
    rule: 'Fold every hand and you still lose steadily to other people’s *Zi Mo* wins. But this table has no penalty for ending a hand not *Ting Pai*, so the pressure to force a *Ting Pai* hand is much weaker here than in the game the rule came from.',
    why: [
  ...
  },
  {
    ...
    rule: 'This table lets a *Zi Mo* win at 1 *Tai* while a discard needs 2. A hand sitting at exactly 1 *Tai* is alive, but nobody can hand it to you — you have to draw it.',
    why: [
  ...
  },
  {
    ...
    why: [
      '*Half Colour* 2, *Full Colour* 4, *Pong Pong* 2, *Ping Wu* 4, a *Dragon* *Pong* or your *Seat Wind* 1 each, and nothing above 5 *Tai* is paid.',
  ...
  },
  {
    ...
    claim: { kind: 'not-countable' },
    verdict: 'measured',
    verdictNote: 'Measured in the data-analytic study of this table. Our own runs disagreed on one number and the reason we gave for it was wrong. Four *Coaches* playing each other draw under 1% of hands against the study’s 14%, and this card used to say that a draw rate is a fact about the players rather than the rules. It is not. Run the same four *Coaches* on the same deals with the *Jokers* taken out and they draw 19.0% instead of 0.6%, and hands run 54 *Turns* instead of 40 — so most of that gap is the *Joker* rule, not the standard of play. Four tiles that stand for anything finish hands that would otherwise die. Everything on this page was measured with four of them in the *Wall*.',
  },
  {
    id: 'mf2_longer_games',
    phase: 'meta',
  ...
  },
  {
    ...
    rule: 'Four plans, four different ways of scoring the same thirteen tiles. The *Pi Wu* counts triplets and pairs, the colour hand counts tiles of one suit and *Honours*, the all-runs hand counts runs and part-runs, and the *Pong Pong* hand is a ratio of triplets to pairs.',
    why: [
  ...
  },
  {
    ...
    why: [
      ...
      'The *Trap* is unchanged, and it is the reason this plan needs deciding early: two suits go on the floor and there is no way back to a *Pi Wu*.',
    ],
    variants: [],
    claim: { kind: 'not-countable' },
    verdict: 'measured',
