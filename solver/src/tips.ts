...
const CARDS: Tip[] = [
  {
    ...
    verdict: 'measured',
    verdictNote: 'The counting is clear on this pair of hands: the five-block one is ready and the six-block one is a tile further away, and it accepts more tiles, which is exactly the trap. The play-outs are not clear at all. On 158 graded positions across both packs holding six blocks where one could be cut for nothing, and where the book’s own exception on the next card did not apply, the measured best cut it 51% of the time against 55% by luck: 56% against 57% at the coach table, 40% against 51% on the old run. So keep the rule for how it makes you SEE a hand, and do not treat the sixth block as an error.',
  },
  {
    id: 'six_blocks_ok',
    phase: 'build',
