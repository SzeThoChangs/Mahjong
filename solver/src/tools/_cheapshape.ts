...
import { CoachBot, NoCheapCoachBot } from '../bot.js';
...
const blank = (): Shape => ({ games: 0, wins: 0, selfDraws: 0, winChips: 0, winTai: 0, dealIns: 0, dealInChips: 0, draws: 0, combos: new Map(), chips: 0 });
...
for (let seat = 0; seat < 4; seat++) { play(seat, () => new NoCheapCoachBot(), arm); play(seat, () => new CoachBot(), coach); }
...
console.log(`  ${''.padEnd(34)} ${'no cheap plan'.padStart(12)}   ${'book coach'.padStart(12)}`);
