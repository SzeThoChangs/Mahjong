...
| `_shapehunt.ts` | where in a real pack does each shape tip fire, and on what hand? Prints a count per tip and one example position, which is what the tagger's tests are built from - a detector tested only against its own card's example proves that it reads that example and nothing more. |
