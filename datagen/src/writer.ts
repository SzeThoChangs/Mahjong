...
export class JsonlGzWriter {
  ...
  constructor(readonly path: string, private chunkBytes = 1 << 20, append = false) { mkdirSync(dirname(path), { recursive: true }); if (!append || !existsSync(path)) writeFileSync(path, ''); }
