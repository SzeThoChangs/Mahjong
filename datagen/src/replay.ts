...
import { makeRules, DEFAULT_RULES } from 'sg-mahjong-engine';
...
export function replayHand(rec: HandRecord, rulesOverride: object = {}, randomness = DEFAULT_RANDOMNESS) {
  const rules = Object.keys(rulesOverride).length ? makeRules(rulesOverride) : DEFAULT_RULES;
...
if (process.argv[1] && /replay\.(ts|js)$/.test(process.argv[1])) {
  ...
  const r = replayHand(rec, manifest.rulesOverride, manifest.randomness);
