...
const MONEY: MoneyRules = { ladder: { 1: 2, 2: 3, 3: 5, 4: 10, 5: 20 }, zm_bonus_per_player: 2, shoot_total: { 2: 7, 3: 11, 4: 20, 5: 40 }, kong_each: 2, kong_fed_total: 6, bite_flower_hidden: 4, bite_flower_open: 2, bite_animal_hidden: 4, bite_animal_open: 2 };
...
function priceFromLedger(ledger: Ledger[], win: { winner: number | null; selfDraw: boolean; discarder: number | null; fan: number; liable: number | null; thirteen: boolean }, m: MoneyRules, r = RULES): number[] {
  ...
  for (let s = 0; s < 4; s++) {
    const l = ledger[s]!;
    out[s] = l.kongEach * m.kong_each + l.kongFed * m.kong_fed_total
...
describe('ledger re-pricing', () => {
  ...
  it('the same ledger prices correctly under a different money schedule', () => {
    const DOUBLE: MoneyRules = { ...MONEY, ladder: { 1: 2, 2: 4, 3: 8, 4: 16, 5: 32 }, shoot_total: { 2: 8, 3: 16, 4: 32, 5: 64 }, kong_each: 5 };
    const cfg = tableConfigOf(RULES);
