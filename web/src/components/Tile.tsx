...
export function Tile({ kind, size = 'md', className, highlight, dim, onClick, badge, fluid }: {
  ...
  // fluid: every tile is the same fraction of the card width (a 14-tile hand fits one row), capped at full size
  const img = (
  ...
  return (
    <div className={cn('relative', fluid ? 'shrink-0 w-[min(3.5rem,6.8cqw)]' : 'inline-block', className)}>
